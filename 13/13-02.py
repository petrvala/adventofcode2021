...
class Board(object):
    ...
    def initialize_board(self, base_points: list[Point]):
        ...
        for point in base_points:
            ...
            if point.y > max_y:
                max_y = point.y
        # i think this is an error in the input data or input description
        max_y = 894
        ...
        print(f"Board initialized with {len(base_points)} points to"
              f" dimensions x: {len(self.board[0])}, y: {len(self.board)}")
    ...
    def make_fold(self, fold: Fold):
        ...
        print(f"New board max x: {len(new_board[0])}, max y: {len(new_board)}")
...
if __name__ == "__main__":
    ...
    for one_fold in folds:
        board.make_fold(fold=one_fold)
        # print(f"x: {len(board.board[0])}, y: {len(board.board)}")
    board.show_board()
